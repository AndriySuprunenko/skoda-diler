    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        @forelse($cars as $car)
            <div class="bg-white rounded-lg shadow hover:shadow-lg transition flex flex-col">
                ...
                @endphp
                <img src="{{ $img }}" alt="{{ $car->name }}" class="rounded-t-lg w-full h-48 object-cover">
                <div class="p-4 flex-1 flex flex-col">
                    <div class="flex items-center gap-2 mb-2">
                        <span
                            class="px-2 py-1 text-xs rounded bg-gray-100 text-gray-700">{{ $car->status ? $statuses[$car->status] ?? $car->status : '' }}</span>
                        <span
                            class="px-2 py-1 text-xs rounded bg-gray-100 text-gray-700">{{ $car->condition ? $conditions[$car->condition] ?? $car->condition : '' }}</span>
                    </div>
